...
void First(vector<Group> &groups) {
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    //   cout << (g.kind == Kind::GOOD ? "GOOD" : "BAD") << endl;
    //   cout << "Units: " << g.units << endl;
    //   cout << "-----------" << endl;
    // }
  }

  if (check_alive(Kind::GOOD)) cout << count_units(Kind::GOOD) << endl;
  if (check_alive(Kind::BAD)) cout << count_units(Kind::BAD) << endl;
  return 0;
}
