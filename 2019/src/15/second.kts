...
var sol: Int = -1
var oxygen: Pair<Int, Int> = 0 to 0

fun dfs(curr: Pair<Int, Int>, steps: Int) {
    for (i in 0..3) {
        val next = (curr.first + directions[i].first) to (curr.second + directions[i].second)
        if (map.containsKey(next)) continue

        val result = runOnce(i + 1)
...
fun main() {
    ...
    val queue: MutableList<Pair<Int, Int>> = mutableListOf(oxygen)
    ...
    while (queue.size > 0) {
        ...
        var iteration: Int = bio.getValue(curr)

        for (i in 0..3) {
            val next = (curr.first + directions[i].first) to (curr.second + directions[i].second)

            if (!map.containsKey(next)) continue
            if (bio.containsKey(next)) continue

            bio.put(next, iteration + 1)
            queue.add(next)
        }
    }

    println(bio.values.max())
