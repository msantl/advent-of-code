...
fun main() {
    ...
    while (true) {
        for (i in 0 until 50) {
            ...
            if (out != null) {
                val curr = outputBuffer.getOrDefault(i, emptyList())
                outputBuffer.put(i, curr.plus(out))

                val buffer = outputBuffer.getOrDefault(i, emptyList())
                if (buffer.size == 3) {
                    val (to, x, y) = buffer.take(3)
                    outputBuffer.put(i, buffer.drop(3))
